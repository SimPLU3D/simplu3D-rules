...
public class CheckOppositeProspect implements IRuleChecker {
  ...
  public CheckOppositeProspect(double slope, double hIni,
      BasicPropertyUnit bPU) {
    super();
    this.slope = slope;
    this.hIni = hIni;
    List<ParcelBoundary> lPB = bPU.getCadastralParcels().get(0)
  ...
  public List<UnrespectedRule> checkOppositeProspect(BasicPropertyUnit bPU,
      List<AbstractBuilding> lBuildings, RuleContext context) {
    ...
    for (AbstractBuilding b : lBuildings) {
      ...
      if (!bool) {
        lUNR.add(new UnrespectedRule("Prospect non respecté", b.getGeom(),
            "Prospect"));
      }
