...
public class LoaderSHPExec {
	...
	public static void main(String[] args) throws Exception {
		...
		for (BasicPropertyUnit bPU : env.getBpU()) {
			...
			IPolygon pol = (IPolygon) FromGeomToSurface.convertGeom(
					bPU.getGeom()).get(0);
			...
			for (CadastralParcel sp : bPU.getCadastralParcel()) {
				...
				AttributeManager.addAttribute(sp, "NBBat", bPU.getBuildings()
						.size(), "Integer");

				for (SpecificCadastralBoundary b : sp
						.getSpecificCadastralBoundary()) {
					bordures.add(b);

					AttributeManager
							.addAttribute(b, "ID", b.getId(), "Integer");
					AttributeManager.addAttribute(b, "Type", b.getType(),
							"Integer");
					AttributeManager.addAttribute(b, "IDPar", sp.getId(),
							"Integer");
					AttributeManager.addAttribute(b, "Type", b.getSide(),
							"Integer");
					...
					if (b.getFeatAdj() != null) {
						...
						if (b.getFeatAdj() instanceof CadastralParcel) {
							...
							AttributeManager.addAttribute(b, "Adj",
									((CadastralParcel) b.getFeatAdj()).getId(),
									"Integer");
					...
					Vecteur v2 = new Vecteur(b.getGeom().coord().get(0), b
							.getGeom().coord()
							.get(b.getGeom().coord().size() - 1));
					v2.setZ(0);
					v2.normalise();
					...
					AttributeManager.addAttribute(feat, "Type", b.getType(),
							"Integer");
					AttributeManager.addAttribute(feat, "Side", b.getSide(),
							"Integer");
		...
		for (SubParcel sp : env.getSubParcels()) {
			...
			AttributeManager.addAttribute(sp, "NB Bat", sp.getBuildingsParts()
					.size(), "Integer");
		}

		ShapefileWriter.write(env.getSubParcels(), folderOut
				+ "sousParcelles.shp");
