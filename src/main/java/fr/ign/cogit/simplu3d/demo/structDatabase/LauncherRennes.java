...
public class LauncherRennes {
	...
	public static List<UnrespectedRule> checkSelection(IFeatureCollection<BasicPropertyUnit> bpuColl) throws Exception {
		...
		for (BasicPropertyUnit currentBPU : bpuColl) {
			...
			List<UnrespectedRule> lUNR = Checker.check(currentBPU, map.get(nomZone));
	...
	private JComboBox<Version> generateCombobox() throws Exception {
		...
		List<Integer> listIdVersion = LoaderVersion.retrieveListIdVersionWithTableVersion(ParametersInstructionPG.host,
				ParametersInstructionPG.port, ParametersInstructionPG.database, ParametersInstructionPG.user,
				ParametersInstructionPG.pw, idUtilisateur);
