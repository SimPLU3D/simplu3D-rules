...
public class LoadFromCollectionPostGIS {
	...
	public static Environnement load(Integer idVersion, IFeatureCollection<IFeature> PLUColl,
		...
		// Here some importers
		UrbaDocument pluImport = ImporterPostGIS.importPLU(PLUColl);
		IFeatureCollection<UrbaZone> zuImport = ImporterPostGIS.importZoneUrba(zoneColl);
		IFeatureCollection<SubParcel> subParImport = ImporterPostGIS.importSubParcel(subParcelsLoad);
		IFeatureCollection<CadastralParcel> cadParImport = ImporterPostGIS.importCadParcel(parcelleColl);
		IFeatureCollection<BasicPropertyUnit> bpuImport = ImporterPostGIS.importBasicPropUnit(bPULoad);
		IFeatureCollection<BuildingPart> bpImport = ImporterPostGIS.importBuildPart(buildingPartLoad);
		IFeatureCollection<SpecificCadastralBoundary> scbImport = ImporterPostGIS.importSpecificCadBound(scbLoad);
		IFeatureCollection<Road> roadImport = ImporterPostGIS.importRoad(voirieColl);
		...
		IFeatureCollection<Building> buildImport = ImporterPostGIS.importBuilding(batiColl);
