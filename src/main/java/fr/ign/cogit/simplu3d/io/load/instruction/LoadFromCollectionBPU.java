...
public class LoadFromCollectionBPU {
  ...
  public static Environnement load(Environnement env, Integer IdVersion,
    ...
    // -------------------------------------------------------------------------
    // Partie 1 : Chargement de la Basic Property Unit ciblée et des objets qui
    // lui sont associés (ses parcelles, ses sous-parcelles, ses batiments...)
    // -------------------------------------------------------------------------
    ...
    // Message d'erreur si la IFC produite et vide (Cas BPU non trouvée dans la
    ...
    // On récupère les identifiants des parcelles importées au sein d'une liste
    ...
    // A partir des SCB de la BPU cible, on récupère les identifiants des routes
    ...
    // On se sert de la liste contenant les identifiants des routes adjacentes
    ...
    // On ajoute les sous-parcelles ainsi chargées à la IFC contenant déjà les
    ...
    // On se sert de la liste des id des sous-parcelles adjacentes pour produire
    // la clause qui servira à l'import des SCB de ces sous-parcelles adjacentes
    // et on lance le chargement et l'import de ces SCB
    String clauseSCBAdj = generatedWhereClauseList(idSubParAdjList,
        attIdSCBSubPar);
    IFeatureCollection<IFeature> scbAdjLoad = PostgisManager
        .loadGeometricTableWhereClause(host, port, database, user, pw,
            tableSCB, clauseSCBAdj);
    IFeatureCollection<SpecificCadastralBoundary> scbAdjImport = ImporterPostGIS
        .importSpecificCadBound(scbAdjLoad);

    // On ajoute les nouvelles SCB à notre IFC contenant les SCB de la BPU cible
    ...
    // On se sert de cette liste d'identifiant pour produire la clause servant à
    // l'import des parcelles adjacentes et on lance le chargement et l'import
    ...
    // On ajoute les nouvelles parcelles à l'IFC contenant les parcelles déjà
    ...
    // produire la clause servant à charger les axes des routes adjacentes et on
    ...
    // On récupère depuis les sous-parcelles chargées les identifiants des zones
    ...
    // charger les zones urba et on lance le chargement et l'import depuis la
    // base de données
    ...
    // On se sert des zones urba chargées pour obtenir les identifiants du PLU
    ...
    // chargement et l'import des parties de batiments depuis la base de données
    ...
    // On lance la création des relations entres tous les objets chargés et on
   ...
   * @param str la valeur de l'attribut à chercher
   * @param nomAtt la nom de l'attribut à chercher
   * @return une clause au format String
   */
   ...
   * @param liste la liste contenant les valeurs à chercher
   * @param nomAtt la nom de l'attribut à chercher
   * @return une clause au format String
   */
  ...
  /**
   * Génère une clause SQL. Spécialement créée pour extraire des buildings parts
   * en fonction d'un id de version et d'un id BPU
   * @param liste La liste des id de sous-parcelles
   * @param nomAttSP Nom de l'attribut pour les sous-parcelles
   * @param idVersion Valeur de l'id version
   * @param nomAttVer Nom de l'attribut pour les version
   * @param nomAttId Nom de l'attribut pour les id
   * @param host
   * @param port
   * @param database
   * @param user
