...
public class LoaderVersion {
	...
	public static void main(String[] args) throws Exception {
		...
		ParametersInstructionPG.database = database;
		...
		// Somes test
		createWhereClauseVersion(host, port, database, user, pw, idVersion);
		retrieveListIdVersionWithoutTableVersion(host, port, database, user, pw);
		retrieveListIdVersionWithTableVersion(host, port, database, user, pw, idUtilisateur);
		...
		// database
		int idIniBuilding = ExtractIdMaxPG.idMaxTable(host, port, database, user, pw,
				ParametersInstructionPG.TABLE_BUILDING, ParametersInstructionPG.ATT_BUILDING_ID);
		...
		IFeatureCollection<IFeature> batiColl = ShapefileReader.read(folder + NOM_FICHIER_BATI);

		// Processing of buildings imported with the BuildingImporter
		IFeatureCollection<Building> buildColl = BuildingImporter.importBuilding(batiColl, collBPU);

		// Update of BuildingPart Table
		searchForNewBuildingPart(host, port, user, pw, database, idIniBuilding, collBPU, buildColl);

		// Load of the buildings shapefile deleted during the versionning
		IFeatureCollection<IFeature> pointColl = ShapefileReader.read(folder + NOM_FICHIER_POINT);

		// Update of Version Table
		searchForBuildingPartsToBeDeleted(pointColl, env, host, port, database, user, pw);

	}

	/**
	 * Permet de mettre à jour les tables en relations avec les Building Parts
	 * en y ajoutant de nouveaux objets
	 * 
	 * @param host
	 * @param port
	 * @param user
	 ...
	 * @param database
	 * @param idIniBuilding
	 *            l'id max de la table Building Part
	 * @param collBPU
	 *            une IFC contenant les BPU
	 * @param buildColl
	 *            une IFC contenant les nouveaux batiments ajoutés par la
	 *            version
	 * @return
	 * @throws Exception
	 */
	public static boolean searchForNewBuildingPart(String host, String port, String user, String pw, String database,
			Integer idIniBuilding, IFeatureCollection<BasicPropertyUnit> collBPU,
		...
		for (BasicPropertyUnit currentBPU : collBPU) {
			...
			for (CadastralParcel currentCP : listCP) {
				...
				for (SubParcel currentSP : featCollSp) {
					...
					IFeatureCollection<AbstractBuilding> featCollBP = currentSP.getBuildingsParts();
						...
						if (idBuildCurrentBP > idIniBuilding) {
							...
							for (Building currentBuilding : buildColl) {
								...
								if (idCurrentBuilding == idBuildCurrentBP) {
									...
									updateTableBuildingPart(host, port, database, user, pw, currentBP, idSP);
									...
									if (currentToit.getRoofing().isEmpty()) {
										...
										updateTableRoofing(host, port, database, user, pw, currentToit);
									}

									if (currentToit.getGutter().isEmpty()) {
										System.out.println(" -- Gutter's empty -- ");
									} else {
										updateTableGutter(host, port, database, user, pw, currentToit);
									}

									if (currentToit.getGable().isEmpty()) {
										System.out.println(" -- Gable's empty -- ");
									} else {
										updateTableGable(host, port, database, user, pw, currentToit);
									}
	...
	/**
	 * Permet de mettre à jour la table version en y ajoutant les id des
	 * batiments supprimés dans une version (calcul par intersection entre des
	 * points en entrée correspondants aux batiments détruits et les polygones
	 * des batiments contenus en base)
	 * 
	 * @param pointColl
	 *            les points correspondants aux batiments détruits
	 * @param env
	 *            l'environnement contenu en base
	 * @param host
	 * @param port
	 * @param database
	 * @param user
	 ...
	 * @throws Exception
	 */
	public static boolean searchForBuildingPartsToBeDeleted(IFeatureCollection<IFeature> pointColl, Environnement env,
			String host, String port, String database, String user, String pw) throws Exception {
		...
		for (IFeature currentPoint : pointColl) {
			...
			for (AbstractBuilding currentBuilding : env.getBuildings()) {
				...
				if (geomPoint.intersects(geomBuild)) {
					...
					updateTableVersion(idVersion, idBuilding, host, port, database, user, pw);
	 ...
	 * @throws Exception
	 */
	public static IFeatureCollection<BasicPropertyUnit> updateCollBPUEnvironnement(Environnement env) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static boolean updateTableVersion(Integer idVersion, Integer idBuild, String host, String port,
			String database, String user, String pw) throws Exception {

		// On cherche l'ID max de la table Version
		int idBIni = (ExtractIdMaxPG.idMaxTable(host, port, database, user, pw, ParametersInstructionPG.TABLE_VERSION,
		...
		String sql_insert = "INSERT INTO " + ParametersInstructionPG.TABLE_VERSION + " ("
				+ ParametersInstructionPG.ATT_VERSION_ID + ", " + ParametersInstructionPG.ATT_VERSION_ID_BUILD_DEL
				+ ", " + ParametersInstructionPG.ATT_VERSION_ID_VERSION_BUILD + ") VALUES (" + idCurrent + ", "
				+ idBuild + ", " + idVersion + ")";

		// On execute la requête dans PostGis à l'aide du PostGisManager
		PostgisManager.executeSimpleRequestInsert(host, port, database, user, pw, sql_insert);
	 ...
	 * @throws Exception
	 */
	public static int updateTableBuilding(String host, String port, String user, String pw, String database,
		...
		String sql_insert = "INSERT INTO " + ParametersInstructionPG.TABLE_BUILDING + " ("
				+ ParametersInstructionPG.ATT_BUILDING_ID + ") VALUES (" + idCurrent + ")";
	 ...
	 * @throws Exception
	 */
	public static boolean updateTableBuildingPart(String host, String port, String database, String user, String pw,
			AbstractBuilding currentBP, Integer idSP) throws Exception {
		...
		String sql_insert = "INSERT INTO " + ParametersInstructionPG.TABLE_BUILDING_PART + " ("
				...
				+ ParametersInstructionPG.ATT_BUILDING_PART_GEOM + ") VALUES (" + idCurrent + ", " + ibBuilding + ", "
				+ idSP + ", " + idversion + ", " + OP_GEOM_FROM_TEXT + "('" + geom + "', " + SRID + " )" + ")";
	 ...
	 * @throws Exception
	 */
	public static boolean updateTableWall(String host, String port, String database, String user, String pw,
			AbstractBuilding currentBP, SpecificWallSurface sws) throws Exception {
		...
		// On prépare la requête pour mettre à jour la table
		String sql_insert = "INSERT INTO " + ParametersInstructionPG.TABLE_WALL_SURFACE + " ("
				+ ParametersInstructionPG.ATT_WALL_SURFACE_ID + ", "
				+ ParametersInstructionPG.ATT_WALL_SURFACE_ID_BUILDP + ", "
				+ ParametersInstructionPG.ATT_WALL_SURFACE_GEOM + ") VALUES (" + idCurrent + ", " + idBP + ", "
	 ...
	 * @throws Exception
	 */
	public static boolean updateTableRoof(String host, String port, String database, String user, String pw,
			AbstractBuilding currentBP, RoofSurface currentToit) throws Exception {

		// On cherche l'ID max de la table Roof
		int idBIni = ExtractIdMaxPG.idMaxTable(host, port, database, user, pw, ParametersInstructionPG.TABLE_ROOF,
		...
		String sql_insert = "INSERT INTO " + ParametersInstructionPG.TABLE_ROOF + " ("
				+ ParametersInstructionPG.ATT_ROOF_ID + ", " + ParametersInstructionPG.ATT_ROOF_ID_BUILDPART + ", "
				+ ParametersInstructionPG.ATT_ROOF_ANGLE_MAX + ", " + ParametersInstructionPG.ATT_ROOF_ANGLE_MIN + ", "
				+ ParametersInstructionPG.ATT_ROOF_GEOM + ") VALUES (" + idCurrent + ", " + idBP + ", " + angleMax
				+ ", " + angleMin + ", " + OP_GEOM_FROM_TEXT + "('" + geom + "', " + SRID + " )" + ")";
	 ...
	 * @throws Exception
	 */
	public static boolean updateTableRoofing(String host, String port, String database, String user, String pw,
			RoofSurface currentToit) throws Exception {

		// On cherche l'ID max de la table Roofing
		int idBIni = ExtractIdMaxPG.idMaxTable(host, port, database, user, pw, ParametersInstructionPG.TABLE_ROOFING,
		...
		String sql_insert = "INSERT INTO " + ParametersInstructionPG.TABLE_ROOFING + " ("
				+ ParametersInstructionPG.ATT_ROOFING_ID + ", " + ParametersInstructionPG.ATT_ROOFING_ID_ROOF + ", "
				+ ParametersInstructionPG.ATT_ROOFING_GEOM + ") VALUES (" + idCurrent + ", " + idToit + ", "
				+ OP_GEOM_FROM_TEXT + "('" + geom + "', " + SRID + " )" + ")";
	 ...
	 * @throws Exception
	 */
	public static boolean updateTableGutter(String host, String port, String database, String user, String pw,
			RoofSurface currentToit) throws Exception {

		// On cherche l'ID max de la table Gutter
		int idBIni = ExtractIdMaxPG.idMaxTable(host, port, database, user, pw, ParametersInstructionPG.TABLE_GUTTER,
		...
		String sql_insert = "INSERT INTO " + ParametersInstructionPG.TABLE_GUTTER + " ("
				+ ParametersInstructionPG.ATT_GUTTER_ID + ", " + ParametersInstructionPG.ATT_GUTTER_ID_ROOF + ", "
				+ ParametersInstructionPG.ATT_GUTTER_GEOM + ") VALUES (" + idCurrent + ", " + idToit + ", "
				+ OP_GEOM_FROM_TEXT + "('" + geom + "', " + SRID + " )" + ")";
	 ...
	 * @throws Exception
	 */
	public static boolean updateTableGable(String host, String port, String database, String user, String pw,
			RoofSurface currentToit) throws Exception {

		// On cherche l'ID max de la table Gable
		int idBIni = ExtractIdMaxPG.idMaxTable(host, port, database, user, pw, ParametersInstructionPG.TABLE_GABLE,
		...
		String sql_insert = "INSERT INTO " + ParametersInstructionPG.TABLE_GABLE + " ("
				+ ParametersInstructionPG.ATT_GABLE_ID + ", " + ParametersInstructionPG.ATT_GABLE_ID_ROOF + ", "
				+ ParametersInstructionPG.ATT_GABLE_GEOM + ") VALUES (" + idCurrent + ", " + idToit + ", "
				+ OP_GEOM_FROM_TEXT + "('" + geom + "', " + SRID + " )" + ")";
	 ...
	 * @throws Exception
	 */
	public static String createWhereClauseVersion(String host, String port, String database, String user, String pw,
			Integer idVersion) throws Exception {

		// On récupère la liste des id à supprimer
		List<Integer> listIdDeleted = searchForIdToDeleteInVersion(host, port, database, user, pw, idVersion);
		...
		if (idVersion == -1) {
			...
			whereClause = AttIdVersion + " = " + Integer.toString(idVersion) + " OR ( " + AttIdVersion + " IS NULL";

			for (Integer valId : listIdDeleted) {

				whereClause = whereClause + " AND " + AttIdBP + " != " + Integer.toString(valId);

			}
			whereClause = whereClause + " )";
		}

		System.out.println("Requête SQL : SELECT * FROM building_part WHERE " + whereClause);
	 ...
	 * @throws Exception
	 */
	public static List<Integer> searchForIdToDeleteInVersion(String host, String port, String database, String user,
			String pw, Integer idVersion) throws Exception {
		...
		IFeatureCollection<IFeature> featColl = PostgisManager.loadNonGeometricTableWhereClause(host, port, database,
				user, pw, table, whereClause);
	 ...
	 * @throws Exception
	 */
	public static List<Integer> retrieveListIdVersionWithoutTableVersion(String host, String port, String database,
		...
		// On récupère le contenu de la table version
		IFeatureCollection<IFeature> featCollVersion = PostgisManager.loadNonGeometricTableWhereClause(host, port,
				database, user, pw, tableVersion, whereClause1);
		...
		IFeatureCollection<IFeature> featCollBP = PostgisManager.loadGeometricTableWhereClause(host, port, database,
				user, pw, tableBuildingPart, whereClause2);
		...
		if (listTempBP.containsAll(listTempV) && (listTempBP.size() == listTempV.size())) {
	 ...
	 * @throws Exception
	 */
	public static List<Integer> retrieveListIdVersionWithTableVersion(String host, String port, String database,
			String user, String pw, Integer idUtilisateur) throws Exception {
		...
		if (idUtilisateur == -1) {
			...
			whereClause = attidUser + " = " + Integer.toString(idUtilisateur) + " ORDER BY " + attidVersion;
		}

		// On charge le contenu de la table user version
		IFeatureCollection<IFeature> featColl = PostgisManager.loadNonGeometricTableWhereClause(host, port, database,
				user, pw, tableUserVersion, whereClause);
	...
	public static IFeatureCollection<BasicPropertyUnit> deleteZCadastralParcel(
		...
		for (BasicPropertyUnit featBPU : collBPU) {
			...
			for (CadastralParcel featCp : featBPU.getCadastralParcel()) {
				...
				List<IOrientableSurface> lIOS = FromGeomToSurface.convertGeom(featCp.getGeom());
	 ...
	 * @return
	 */
	public static IFeatureCollection<IFeature> deleteZBuilding(IFeatureCollection<IFeature> batiColl) {

		for (IFeature featBatid : batiColl) {

			List<IOrientableSurface> lIOS = FromGeomToSurface.convertGeom(featBatid.getGeom());
