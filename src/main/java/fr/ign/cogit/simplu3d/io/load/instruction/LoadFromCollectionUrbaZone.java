...
public class LoadFromCollectionUrbaZone {
  ...
  public static Environnement load(Environnement env, AbstractDTM dtm,
    ...
    if (zuImport.isEmpty()) {
      System.out.println("\n" + "ERREUR : La valeur '" + searchIdZU
          + "' n'existe pas dans la table " + tableZU + ".");
      return null;
    }
    ...
    IFeatureCollection<IFeature> cpLoad = PostgisManager
        .loadGeometricTableWhereClause(host, port, database, user, pw,
            tableCadPar, clauseCP);
    IFeatureCollection<CadastralParcel> cadParImport = ImporterPostGIS
        .importCadParcel(cpLoad);
    ...
    if (!idSubParAdjList.isEmpty()) {
      ...
      String clauseSPAdj = generatedWhereClauseList(idSubParAdjList,
          attIdSubPar);
      ...
      String clauseCPAdj = generatedWhereClauseList(idParCadAdjList,
          attIdCadPar);
      ...
      IFeatureCollection<IFeature> zuAdjLoad = PostgisManager
          .loadGeometricTableWhereClause(host, port, database, user, pw,
              tableZU, clauseZUAdj);
      IFeatureCollection<UrbaZone> zuAdjImport = ImporterPostGIS
          .importZoneUrba(zuAdjLoad);
