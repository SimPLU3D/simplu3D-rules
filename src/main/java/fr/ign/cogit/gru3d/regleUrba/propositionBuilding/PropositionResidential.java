 ...
 * @version 1.0
 **/
public class PropositionResidential {

  private static int itMax = 5000;
  private final static Logger logger = Logger
      .getLogger(PropositionResidential.class.getName());
  ...
  public static Batiment proposition(Parcelle p, Moteur m) {
    ...
    double cesMax = Double.NEGATIVE_INFINITY;

    Batiment client = null;

    while (iteration++ < itMax) {

      // logger.info("Nombre d'itération : " + iteration);
    ...
    if (client != null) {
      ...
      if (!Export.exportAll) {
        Export.export(client, p, "CES=" + cesMax);
      }
  ...
  private static Batiment generateBuildingType1(Parcelle p) {
    ...
    // logger.info("Génération du type : " + (System.currentTimeMillis() -
    // time));
    time = System.currentTimeMillis();
    ...
    // logger.info("Choix du centre : " + (System.currentTimeMillis() - time));
    time = System.currentTimeMillis();
    ...
    double longueur = largeur * (1 + Math.random() / 2);

    // Quel polygone ?
    IPolygon poly = propositionPolygon1(fB, centre, largeur, longueur);

    // logger.info("Génération polygone : " + (System.currentTimeMillis() -
    // time));
    time = System.currentTimeMillis();
    ...
    // logger.info("Orientation du polygone : " + (System.currentTimeMillis() -
    // time));
    time = System.currentTimeMillis();
    ...
    // logger.info("Génération du toit : " + (System.currentTimeMillis() -
    // time));
    time = System.currentTimeMillis();
  ...
  private static IDirectPositionList generateGrid1(Parcelle p) {
    ...
    List<Route> lR = p.getlRouteBordante();
