...
public class ContrainteCOSRepresentation extends Default3DRep {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      feat.setRepresentation(new ContrainteCOSRepresentation(feat, 70, 0.0,
          1.2, 2.0, Color.pink, 50, url.getPath(), 3.0, 10));
      /*
  ...
  public ContrainteCOSRepresentation(IFeature feat, double hauteur,
      double cosMin, double cosActu, double cosMax, Color coul, double width,
      String imgPath, double lineWidth, double tailleText) {
    super();
    this.feat = feat;
  ...
  public static BranchGroup generateBG(GM_Point p, double hauteur, Color coul,
      double width, String cosMin, String cosActu, String cosMax,
      double lineWidth, double tailleText, String texture) {

    Color3f col3f = new Color3f(coul);

    LineStripArray lsa = ContrainteCOSRepresentation.geometryWithColor(p,
        hauteur, col3f);
    BranchGroup bh = ContrainteCOSRepresentation.generateCube(p.getPosition(),
        width, col3f, cosMin, cosActu, cosMax, hauteur, tailleText, texture);
    BranchGroup bGRep = new BranchGroup();

    bGRep.addChild(new Shape3D(lsa, ContrainteCOSRepresentation.lineAppearence(
        col3f, (float) lineWidth)));
    bGRep.addChild(bh);
    return bGRep;
  }
  ...
  private static BranchGroup generateCube(IDirectPosition dp, double width,
      Color3f coul, String cosMin, String cosActu, String cosMax,
      double hauteur, double tailleText, String texture) {
  ...
  private static Group generateLocalText(String text, Color3f coul,
      double tailleText, double shift) {
    ...
    Shape3D s3D1 = new Shape3D(text3D,
        ContrainteCOSRepresentation.genereApparenceText(coul, 1.0));
    ...
    tg1.set(new Vector3f((float) 0.0, (float) 0.0, (float) shift));
