...
public class ParisProposition {
   ...
   * @return
   */
  private static FormeBatiment getType(Parcelle p) {

    int nbVoisRoute = p.getlRouteBordante().size();
  ...
  private static IPolygon propositionPolygon(FormeBatiment fB,
    ...
    double largeur = 10 + 10 * r.nextGaussian();
    double hauteur = 30 + 30 * r.nextGaussian();
    ...
    for (double x = xmin; x <= xmax; x = x + 0.5) {
      for (double y = ymin; y <= ymax; y = y + 0.5) {
        ...
        if (pol.intersects(new GM_Point(dp))) {
          ...
          if (pol.distance(new GM_Point(dp)) < 4) {
            dpl.add(dp);
          }
  ...
  private static IPolygon propositionOrientedRotation(IPolygon poly, Parcelle p) {
    ...
    for (Route rTemp : lR) {
      ...
      if (r.getGeom().distance(poly) < rTemp.getGeom().distance(poly)) {
    ...
    IRing ext = ((IPolygon) r.getGeom()).getExterior();
  ...
  private static IMultiSurface<IPolygon> generateRoof(IPolygon poly,
    ...
    double zMax = zGut + (int) (4 * Math.random() + 4);
